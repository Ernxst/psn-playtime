...
const ServerLayer = Layer.mergeAll(AppConfig.layer, TitleEnrichmentLayer);
 ...
 * `TitleEnrichment` is satisfied by the process-lived `ServerLayer`, so
 * enrichment handlers no longer provide it per request.
 */
export const runServer = <A, E>(
  effect: Effect.Effect<A, E, AppConfig | TitleEnrichment>
