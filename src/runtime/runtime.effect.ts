 ...
 * `DashboardSource` stays out of here — it carries a transient credential and is
 * provided at its own handler boundary.
 */
const serverRuntime = ManagedRuntime.make(TitleEnrichmentLayer);
 ...
 * `TitleEnrichment` is satisfied by the process-lived `serverRuntime`, so
 * enrichment handlers no longer provide it per request.
 */
export const runServer = <A, E>(effect: Effect.Effect<A, E, TitleEnrichment>): Promise<A> =>
