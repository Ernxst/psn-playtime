 ...
 * un-enriched `DashboardData` contract — behind the platform-agnostic seam so a
 * future Xbox provider satisfies the same shape.
 *
 * Structure after the #165 hardening:
 * - The authenticated PSN session is a `PsnClient` `Context.Service`
 *   (`./psn/psn-client.effect`): it does the npsso→token exchange once and
 *   captures `auth` in closure, so `auth` is never threaded as a parameter here.
 * - The pure normalization/name-matching helpers live in `./psn/psn-normalize`
 ...
 */
const buildSnapshot: Effect.Effect<DashboardData, AccountProviderError, PsnClient> = Effect.gen(
  function* () {
